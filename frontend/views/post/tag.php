...
$this->title                   = $data['breadword'].' | '.yii::$app->params['bandName'] . Yii::$app->params['suffix'];
?>
...
foreach ($data['body'] as $list): ?>
    ...
    <div class="col-lg-8 col-md-8 col-sm-8 col-mx-8">
        ...
        <?php foreach ($list['tags'] as$k=>$tag):?>
         <span><a href="<?=Url::to(['post/tag','id'=>$k])?>">
            <?php if ($data['breadword']==$tag) {?>
                <b class="lightword"><?=$tag?></b>
           <?php } else { ?>
                <?=$tag?>
           <?php }?>
            </a></span>&nbsp;
...
<div><?=LinkPager::widget(['pagination' => $data['page']]);?></div>
</div>
